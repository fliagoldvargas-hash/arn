...
async function main() {
  ...
  const dir = path.join(root, "docs", "design-references", "local-marketplace");
  await fs.mkdir(dir, { recursive: true });
  const browser = await chromium.launch({ headless: true });

  const desktop = await browser.newPage({ viewport: { width: 1440, height: 1200 }, deviceScaleFactor: 1 });
  await desktop.goto("http://127.0.0.1:3000/marketplace", { waitUntil: "networkidle", timeout: 60000 });
  ...
  await mobile.goto("http://127.0.0.1:3000/marketplace", { waitUntil: "networkidle", timeout: 60000 });
  await mobile.screenshot({ path: path.join(dir, "mobile-full.png"), fullPage: true });

  await browser.close();
  console.log("local marketplace qa screenshots saved");
}
